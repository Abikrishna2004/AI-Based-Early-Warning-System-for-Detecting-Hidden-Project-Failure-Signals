"""
Project Sentinel - Round-Robin Size-Balanced 5-Fold Cross-Validation Script
=============================================================================
1. Identifies the isolated project in Fold 1 of standard GroupKFold and inspects its label distribution.
2. Performs exact Round-Robin assignment on projects sorted by row count descending (Project 1 -> Fold 1, Project 2 -> Fold 2, etc.).
3. Evaluates 5-fold cross-validation of baseline CatBoost on these Round-Robin folds.
...
def run_round_robin_cv():
    print("=" * 90, flush=True)
    print("        PROJECT SENTINEL - ROUND-ROBIN 5-FOLD CROSS-VALIDATION", flush=True)
    print("=" * 90, flush=True)
    ...
    # 3. Construct Round-Robin Group Assignment (Sorted by Row Count Descending)
    print("\n[3/5] Constructing Round-Robin Group Assignment (Sorted by Row Count Descending)...", flush=True)
    ...
    # Assign 0..4 round-robin across sorted projects
    proj_counts['rr_fold'] = [i % 5 for i in range(len(proj_counts))]
    proj_to_fold = dict(zip(proj_counts['project_id'], proj_counts['rr_fold']))
    df['rr_fold'] = df['project_id'].map(proj_to_fold)

    print("\n      --- Round-Robin Fold Summary ---", flush=True)
    for f_idx in range(5):
        f_df = df[df['rr_fold'] == f_idx]
    ...
    # 4. Run Round-Robin 5-Fold Cross-Validation
    print("\n[4/5] Running 5-Fold Cross-Validation on Round-Robin Folds...", flush=True)
    rr_results = []
    start_cv = time.time()

    for fold_num in range(1, 6):
        f_idx = fold_num - 1
        val_mask = df['rr_fold'] == f_idx
        tr_mask = ~val_mask
    ...
    print(f"\n[5/5] Round-Robin Cross-Validation completed in {cv_duration/60:.2f} minutes.", flush=True)

    # 5. Output Final Results & Statistics
    res_df = pd.DataFrame(rr_results)

    accs = [r['Accuracy'] for r in rr_results]
    macro_f1s = [r['Macro F1'] for r in rr_results]
    high_recs = [r['High Recall'] for r in rr_results]
    ...
    print("\n=========================================================================================================")
    print("                    ROUND-ROBIN 5-FOLD PROJECT-GROUPED CROSS-VALIDATION RESULTS")
    print("=========================================================================================================")
    print(res_df.to_string(index=False))
    print("=" * 105)
    print(f"  Round-Robin 5-Fold CV Mean ± Std Accuracy       : {mean_acc:.4f} ± {std_acc:.4f}")
    print(f"  Round-Robin 5-Fold CV Mean ± Std Macro F1       : {mean_macro_f1:.4f} ± {std_macro_f1:.4f}")
    print(f"  Round-Robin 5-Fold CV Mean ± Std High-Risk Recall: {mean_high_rec:.4f} ± {std_high_rec:.4f}")
    print("=========================================================================================================\n", flush=True)
